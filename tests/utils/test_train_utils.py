...
def test_explicit_checkpoint_steps_override_periodic_schedule():
    explicit_steps = [35_000, 40_000, 50_000]

    assert not should_save_checkpoint(5_000, 50_000, 5_000, explicit_steps)
    assert should_save_checkpoint(35_000, 50_000, 5_000, explicit_steps)
    assert should_save_checkpoint(40_000, 50_000, 5_000, explicit_steps)
    assert should_save_checkpoint(50_000, 50_000, 5_000, explicit_steps)


def test_full_training_state_can_be_limited_to_selected_steps():
    full_steps = [40_000]

    assert not should_save_training_state(35_000, full_steps)
    assert should_save_training_state(40_000, full_steps)
    assert not should_save_training_state(50_000, full_steps)
...
@patch("lerobot.common.train_utils.save_training_state")
def test_save_checkpoint_can_skip_training_state(mock_save_training_state, tmp_path, optimizer):
    policy = Mock()
    cfg = Mock()

    save_checkpoint(tmp_path, 10, cfg, policy, optimizer, save_training_state_file=False)
