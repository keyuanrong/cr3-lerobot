...
| 策略 | 在 CR3 项目中的能力 | 适用场景 |
| --- | --- | --- |
| ACT | 支持双相机观测、机械臂状态输入、动作预测与 CR3 真机执行 | 端到端模仿学习与真机控制 |
| SmolVLA | 支持视觉-语言条件动作生成，可接入 CR3 控制与统一仿真验证流程 | 轻量化 VLA 实验与仿真验证 |
| Pi0 / OpenPI | 支持语言任务、双相机输入、动作块预测、远程 GPU 推理与 CR3 执行 | 视觉语言动作学习、远程推理与真机部署 |
| MuJoCo 仿真环境 | 为 ACT、SmolVLA、Pi0 等策略提供统一的 CR3 + LMG-90 场景、回放、关节映射与控制验证 | 策略调试、回放验证与 sim-to-real 前检查 |
...
- 定点位附近表现相对稳定，但物块颜色、初始位置、机械臂初始姿态或遮挡发生变化时，抓取成功率明显波动。
- 随机物体位置下，末端能够到达物体上方，但下探深度、夹爪打开/闭合时机和抬起动作仍不稳定。
- 长任务会累积误差，前一阶段偏差可能导致后续搬运和放置失败。
