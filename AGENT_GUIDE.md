...
对 CR3 Pi0 模型，可将验证集保留在本地，只把每次模型请求发送到局域网 3090 的 `pi0_remote_policy_server.py`。下面的命令不连接机械臂；它以 `30 Hz` 控制、`1.5 Hz` 请求、action chunk、延迟补偿和 ensemble 队列回放验证轨迹。

```bash
/home/kyr/miniconda3/envs/lerobot/bin/python -m scripts.evaluation.eval_remote_pi0_replay \
  --server 192.168.1.125:8765 \
  --dataset 'local/cr3_pi0_unified_goal_v1_validation_complete_goal=lerobot_data/local/cr3_pi0_unified_goal_v1_validation_complete_goal' \
  --dataset 'local/cr3_pi0_unified_goal_v1_validation_goal_grasp_event=lerobot_data/local/cr3_pi0_unified_goal_v1_validation_goal_grasp_event' \
  --dataset 'local/cr3_pi0_unified_goal_v1_validation_goal_place_event=lerobot_data/local/cr3_pi0_unified_goal_v1_validation_goal_place_event' \
  --dataset 'local/cr3_pi0_unified_goal_v1_validation_atomic_assist=lerobot_data/local/cr3_pi0_unified_goal_v1_validation_atomic_assist' \
  --output-dir outputs/eval/pi0_full_unified_goal_050000
```

查看 `summary.json` 中的 `open_recall`、`close_recall`、关节 `joint_mae` 和 `joint_p95_abs_error`。离线回放使用录制的后续画面，不能替代真机成功率，但能识别模型是否在未见轨迹上仍持续输出错误的夹爪状态。
