...
def validate_gradient_accumulation_steps(gradient_accumulation_steps: int) -> None:
...
class EarlyStoppingConfig:
    enable: bool = False
    # Do not stop before this many optimizer steps.
    min_steps: int = 0
    # Stop if the smoothed loss has not improved for this many optimizer steps.
    patience_steps: int = 10_000
    # Minimum loss decrease required to count as an improvement.
    min_delta: float = 1e-4
    # Number of logged loss points to average before checking improvement.
    window: int = 5


@dataclass
class DatasetMixtureConfig:
    """Six unified-goal datasets with fixed phase and target-task sampling quotas."""

    repo_ids: list[str]
    root: str
    weights: list[float]
    block_size: int = 1_000
    balance_task_labels: bool = True
    ...
    def validate(self) -> None:
        if len(self.repo_ids) != 6 or len(self.weights) != 6:
            raise ValueError("Pi0 unified-goal mixture requires exactly 6 datasets and 6 weights.")
        if any(not repo_id.strip() for repo_id in self.repo_ids):
            raise ValueError("Mixture dataset repo IDs must be non-empty.")
        if not self.root.strip():
            raise ValueError("Mixture dataset root must be non-empty.")
        ...
        if self.block_size <= 0:
            raise ValueError("Mixture block_size must be positive.")
...
class TrainPipelineConfig(HubMixin):
    ...
    mixture: DatasetMixtureConfig | None = None
    ...
    # Optional explicit checkpoint schedule. When set, it overrides `save_freq` and the implicit final save.
    checkpoint_steps: list[int] | None = None
    # Optional subset of `checkpoint_steps` that retains optimizer state for resuming training.
    # Other checkpoints contain only the deployable pretrained policy.
    full_checkpoint_steps: list[int] | None = None
    ...
    def validate(self) -> None:
        validate_gradient_accumulation_steps(self.gradient_accumulation_steps)
        ...
        if self.mixture is not None:
            ...
            if self.dataset.streaming:
                raise ValueError("Mixture training does not support streaming datasets.")
