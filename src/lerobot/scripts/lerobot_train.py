...
def mean_microbatch_losses(losses: list[AverageMeter]) -> float:
    """Return the numeric mean of the current loss from accumulated microbatches."""
    return sum(loss.val for loss in losses) / len(losses)


def format_policy_auxiliary_losses(output_dict: dict[str, Any] | None) -> str | None:
    """Format Pi0 flow and trajectory-consistency losses for the local training log."""
    loss_keys = ("flow_loss", "action_velocity_loss", "action_acceleration_loss")
    if not output_dict or any(key not in output_dict for key in loss_keys):
        return None
    return "policy_losses " + " ".join(f"{key}:{float(output_dict[key]):.6f}" for key in loss_keys)
...
def update_policy(
    ...
    if accelerator.sync_gradients:
        # Gradients now contain the accumulated microbatches.
        if grad_clip_norm > 0:
        ...
        # Scheduler and policy buffers advance once per optimizer update.
...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    if is_main_process:
        ...
        logging.info(
            "Effective batch size: %s (physical batch) x %s (accumulation) x %s (processes) = %s",
            cfg.batch_size,
        ...
        if cfg.mixture is not None:
            logging.info(
                "Fixed-ratio mixture: repo_ids=%s weights=%s block_size=%s task_balancing=%s",
                cfg.mixture.repo_ids,
                cfg.mixture.normalized_weights,
                cfg.mixture.block_size,
                cfg.mixture.balance_task_labels,
            )

    # create dataloader for offline training
    if cfg.mixture is not None:
        if accelerator.num_processes != 1:
            raise NotImplementedError("Fixed-ratio mixture training currently supports one process only.")
        shuffle = False
        if cfg.mixture.balance_task_labels:
            ...
            sampler = FixedQuotaBucketSampler(
                ...
                num_samples=(cfg.steps - step) * cfg.batch_size * cfg.gradient_accumulation_steps,
                block_size=cfg.mixture.block_size,
                seed=cfg.seed or 0,
            )
            if is_main_process:
                logging.info("Using fixed phase-and-target quotas: %s", UNIFIED_GOAL_BUCKET_QUOTAS)
        else:
            sampler = FixedRatioMixtureSampler(
                dataset_lengths=[child.num_frames for child in dataset._datasets],
                weights=cfg.mixture.normalized_weights,
                num_samples=(cfg.steps - step) * cfg.batch_size * cfg.gradient_accumulation_steps,
                block_size=cfg.mixture.block_size,
                seed=cfg.seed or 0,
            )
    ...
    # Count all microbatches that contribute to one optimizer update.
    ...
    early_stop_losses: list[float] = []
    early_stop_best_loss = float("inf")
    early_stop_best_step = step
    ...
    for _ in range(step, cfg.steps):
        ...
        for _ in range(cfg.gradient_accumulation_steps):
            ...
            microbatch_losses.append(train_tracker.loss)

        train_tracker.loss = mean_microbatch_losses(microbatch_losses)
        ...
        is_saving_step = should_save_checkpoint(
            step, cfg.steps, cfg.save_freq, cfg.checkpoint_steps
        )
        is_eval_step = cfg.eval_freq > 0 and step % cfg.eval_freq == 0

        if is_log_step:
            logged_loss = float(train_tracker.loss.avg)
            logging.info(train_tracker)
            auxiliary_losses = format_policy_auxiliary_losses(output_dict)
            if auxiliary_losses:
                logging.info(auxiliary_losses)
            if wandb_logger:
            ...
            if cfg.early_stopping.enable:
                early_stop_losses.append(logged_loss)
                if len(early_stop_losses) > cfg.early_stopping.window:
                    early_stop_losses.pop(0)
                if len(early_stop_losses) == cfg.early_stopping.window:
                    smoothed_loss = sum(early_stop_losses) / len(early_stop_losses)
                    improved = smoothed_loss < early_stop_best_loss - cfg.early_stopping.min_delta
                    if improved:
                        early_stop_best_loss = smoothed_loss
                        early_stop_best_step = step
                    elif (
                        step >= cfg.early_stopping.min_steps
                        and step - early_stop_best_step >= cfg.early_stopping.patience_steps
                    ):
                        early_stop_requested = True
                        logging.info(
                            "Early stopping triggered at step %s: smoothed_loss=%.6f, "
                            "best_smoothed_loss=%.6f at step %s, no improvement for %s steps",
                            step,
                            smoothed_loss,
                            early_stop_best_loss,
                            early_stop_best_step,
                            step - early_stop_best_step,
                        )
            train_tracker.reset_averages()

        if cfg.save_checkpoint and (is_saving_step or early_stop_requested):
            if is_main_process:
                if early_stop_requested and not is_saving_step:
                    logging.info(f"Checkpoint policy after step {step} before early stop")
                else:
                    logging.info(f"Checkpoint policy after step {step}")
                checkpoint_dir = get_step_checkpoint_dir(cfg.output_dir, cfg.steps, step)
                is_full_checkpoint = early_stop_requested or should_save_training_state(
                    step, cfg.full_checkpoint_steps
                )
                save_checkpoint(
                    ...
                    save_training_state_file=is_full_checkpoint,
                )
