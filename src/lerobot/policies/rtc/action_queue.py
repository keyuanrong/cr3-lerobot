...
class ActionQueue:
    ...
    def merge_with_consumed_delay(
        self,
        original_actions: Tensor,
        processed_actions: Tensor,
        action_index_before_inference: int,
    ) -> int:
        """Atomically replace an RTC queue using actions consumed during inference.

        This is for control loops where another thread consumes actions while
        inference is running. Computing the delay before acquiring this lock
        leaves a one-tick race between the calculation and queue replacement.

        Returns:
            Number of actions consumed between request start and replacement.
        """
        with self.lock:
            real_delay = max(0, self.last_index - action_index_before_inference)
            if self.cfg.enabled:
